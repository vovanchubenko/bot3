...
BONUS = os.environ["BONUS"]
WITHDRAW = os.environ["WITHDRAW"]
REFER = os.environ["REFER"]
##TOKEN DETAILS
TOKEN = os.environ["CURRENCY"]

BOT_TOKEN = os.environ["TOKEN"]
PAYMENT_CHANNEL = os.environ["PAYMENT_CHANNEL"] #add payment channel here including the '@' sign
OWNER_ID = OWNER #write owner's user id here.. get it from @MissRose_Bot by /id
CHANNELS = ["@MJBotTest"] #add channels to be checked here in the format - ["Channel 1", "Channel 2"] 
              #you can add as many channels here and also add the '@' sign before channel username
Daily_bonus = BONUS #Put daily bonus amount here!
Mini_Withdraw = WITHDRAW #remove 0 and add the minimum withdraw u want to set
Per_Refer = REFER #add per refer bonus here
...
def amo_with(message):
   try:
    ...
    send = bot.send_message(PAYMENT_CHANNEL,  "✅* New Withdraw\n\n⭐ Amount - "+str(amo)+f" {TOKEN}\n🦁 User - @"+message.from_user.username+"\n💠 Wallet* - `"+data['wallet'][user]+"`\n☎️ *User Referrals = "+str(
        ...
        bot.send_message(OWNER_ID, "Your bot got an error fix it fast!\n Error on command: "+message.text)
        return
  r = requests.post('https://full2sms.in',data = pload)
print(r.text)
